...
const showcaseContainers = ["", "", "-mobile"];
...
function popUp(idx, close = true) {
    ...
    document.getElementById("album-showcase-description" + w).innerText =
        entry.description;
    document.getElementById("album-showcase-cover" + w).src =
        entry.img_location;
    ...
    document.getElementById("button-left" + w).onclick = function () {
        nextShowcase(prev);
    };
    document.getElementById("button-right" + w).onclick = function () {
        nextShowcase(next);
    };
...
fetch("./assets/scripts/discography.json")
    ...
    .then((data) => {
        ...
        data.forEach((entry) => {
            const discogCard =
                discogTemplate.content.cloneNode(true).children[0];
            ...
            const discogSubHeader =
                discogCard.querySelector("[discog-subheader]");
...
window.addEventListener("resize", function () {
    ...
    if (width !== windowWidth) {
        const oldEnt = document.getElementById(
            "album-showcase-container" + showcaseContainers[windowWidth]
        );
